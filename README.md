...
Lastly, all we have to do, is to add the songs’ durations for each pair (using two merge_ commands), generate their sum, and with a simple list-command find pairs that match the time we are looking for. Since the duration is measured in milliseconds you have to convert the time you are looking for first by the formula _minutes x 60,000 x 1,000_. I furthermore recommend allowing at least one second (1,000 milliseconds) +/- that time, to ensure you will find enough results.
...
There are already 25 pairs of songs to choose from. You might argue that some of those are wild concoctions, musically, like combining energetic _Overdone_ with a sweet acoustic guitar based _Fairytale Lullaby_. And I agree! You could handpick songs that you would like to keep (or drop) in step one to avoid this, but you would also decrease the number of possible pairs of songs with this. If you allowed for an additional second +/- your desired time, the number of possible pairs doubles to triples, depending on the specific interval. Here’s an overview of how many suggestions I got for each time-interval:
